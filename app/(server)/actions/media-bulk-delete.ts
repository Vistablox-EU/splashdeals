...
export async function bulkDeleteMediaAction(
  ...
  try {
    ...
    const missingIds = validation.data.mediaIds.filter((id) => !foundIds.has(id));
    if (missingIds.length > 0) {
      throw new Error(`Some media items not found: ${missingIds.join(", ")}`);
    }
      ...
      async (tx) => {
        ...
        for (const media of mediaItems) {
          try {
            ...
            console.error(`[Bulk Delete] Failed to delete blob ${media.url}:`, error);
